...
syntaxNode * currentFunction = new syntaxNode();
...
syntaxNode * parseSyntax(syntaxNode * syntax, std::vector<syntaxNode*> arguments, std::vector<syntaxNode*> & currentScope)
{
  ...
  if (syntax->_type != "literal")
  {
    ...
    {
      return new syntaxNode("literal", concatenateMethod(scriptVariables, syntax, arguments, currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "input")
    {
      return new syntaxNode("literal", inputMethod(scriptVariables, arguments),syntax->_syntax);
    }
    else if (syntax->_syntax == "add")
    {
      return new syntaxNode("literal", addNums(scriptVariables, syntax, arguments, currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "subtract")
    {
      return new syntaxNode("literal", subtractNums(scriptVariables, syntax, arguments, currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "multiply")
    {
      return new syntaxNode("literal", multiplyNums(scriptVariables, syntax, arguments, currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "divide")
    {
      return new syntaxNode("literal", divideNums(scriptVariables, syntax, arguments, currentScope),syntax->_syntax);
    }
    ...
    {
      return new syntaxNode("literal", readVarIndexMethod(scriptVariables[arguments[0]->_syntax], scriptVariables,syntax,arguments,currentScope),syntax->_syntax);// sets arg 0 variable to arg 1
    }
    else if (syntax->_syntax == "equal")
    {
      return new syntaxNode("literal", equal(scriptVariables,syntax,arguments,currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "greater")
    {
      return new syntaxNode("literal", greater(scriptVariables,syntax,arguments,currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "less")
    {
      return new syntaxNode("literal", less(scriptVariables,syntax,arguments,currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "lessOrEqual")
    {
      return new syntaxNode("literal", lessOrEqual(scriptVariables,syntax,arguments,currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "greaterOrEqual")
    {
      return new syntaxNode("literal", greaterOrEqual(scriptVariables,syntax,arguments,currentScope),syntax->_syntax);
    }
    else if (syntax->_syntax == "exec")
    {
      return new syntaxNode("literal", functionInstantiateMethod(scriptVariables, syntax, functions, arguments),syntax->_syntax);
    }
    else if (syntax->_syntax == "return")
    {
      return new syntaxNode("literal", parseArguments(scriptVariables, 0, syntax, arguments, currentScope), "returning");
    }
    ...
    {
      return new syntaxNode("literal", itterateScope(functions[syntax->_syntax], syntax->_arguments), syntax->_syntax);
    }
  }
  return syntax;
}
    ...
    for (std::size_t i = 0; i < arguments.size(); ++i)
    {
      ...
      else
      {
        if (arguments[i]->_scope.empty())//scope empty
        {
          ...
          {
            returnVec.push_back( new syntaxNode("literal", scriptVariables[arguments[i]->_syntax]->getVariableValue()[0],arguments[i]->_syntax));
          }
...
std::string itterateScopeRecursion(syntaxNode * currentScope, std::vector<syntaxNode*> & userArguments)
{
  ...
  //SCOPES
  //if (currentScope._scope.size() > currentScope.scopeIndex)std::cout << currentScope._scope[currentScope.scopeIndex]._type << " current type!\n";
